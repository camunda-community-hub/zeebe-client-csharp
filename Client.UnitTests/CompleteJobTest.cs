...
namespace Zeebe.Client
{
    ...
    public class CompleteJobTest : BaseZeebeTest
    {
        ...
        public async Task ShouldRetrySendRequestOnResourceExhaustedUntilSucceed()
        {
            ...
            // when
            var resultTask = ZeebeClient.NewCompleteJobCommand(12113).SendWithRetry();
            countdownEvent.Wait(TimeSpan.FromSeconds(10));

            // then
            Assert.AreEqual(0, countdownEvent.CurrentCount);
            TestService.AddRequestHandler(typeof(CompleteJobRequest), req => new CompleteJobResponse());
            await resultTask;

            var request = TestService.Requests[typeof(CompleteJobRequest)][0];
            Assert.AreEqual(expectedRequest, request);

            var requestCount = TestService.Requests[typeof(CompleteJobRequest)].Count;
